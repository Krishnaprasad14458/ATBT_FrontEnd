...
function Users() {
  ...
  let keys = Object.keys(settings?.paginatedUsers?.[0] ?? {});
  console.log(keys, 'keys');

  const loopArray = Array.from({
    length: Object.keys(settings?.paginatedUsers?.[0] ?? {}).length,
  });
            ...
            <tbody className='divide-y divide-gray-200 dark:divide-gray-700'>
              {settings?.paginatedUsers?.map((user) => (
                <tr className='hover:bg-gray-100 dark:hover:bg-gray-700'>
                  ...
                  <td className='px-6 py-2 text-left border border-[#e5e7eb] text-xs font-medium text-gray-800'>
                    {user.EntityName ? user.EntityName : 'none'}
                  </td>
                  <td className='px-6 py-2 text-left border border-[#e5e7eb] text-xs font-medium text-gray-800'>
                          ...
                          >
                            ...
                            <div
                              className={`ml-3 text-sm font-medium ${
                                isChecked ? 'text-gray-400' : 'text--400'
                              }`}
                            >
              ...
              {/* old code commented */}
              {/* {settings &&
                settings?.paginatedUsers.map((item, index) => (
                  <tr>
                    {loopArray.map(
                      (items, index) =>
                        tableView[keys[index]] && (
                          <td className='px-6 py-2 text-left border border-[#e5e7eb] text-xs font-medium text-gray-800'>
                            {item[keys[index]]}
                          </td>
                        )
